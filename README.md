...
AimiliVPN 是一款基于官方 VPNGate 开放协议的高性能、零依赖 VPN 代理网关。它以纯 Python 标准库编写，内置 Apple 风格的浅色响应式管理网页，提供智能并发测速、多路由模式、出站代理网关、实时日志等功能。
...
#### 🌟 正式稳定版本（main 分支）
```bash
bash <(curl -Ls https://raw.githubusercontent.com/SkywardLab/aimili-vpngate/main/install.sh)
```
...
#### 第一步：登录 Web 管理后台
打开浏览器，访问部署完成时提示的专属后台地址（含安全后缀），即可进入 Apple 风格的浅色管理界面。
...
* **网关状态面板**：
  - **系统诊断**：检测网关心跳及后台各个子守护线程（网页服务、VPN 连接管理、出站网关服务）是否正常运行。若有脚本未运行，会提示具体的异常原因。
...
* **解决办法 2**：**非常重要！** 登录云服务商控制台（如阿里云、腾讯云、AWS、Oracle Cloud 等），找到您 VPS 实例的 **安全组规则 (Security Group)**，在入站规则中添加：
  - **协议类型**：`TCP`
  - **端口范围**：`8787`（管理网页）和 `7928`（代理端口）
  - **授权对象/源 IP**：`0.0.0.0/0`（允许所有人，或指定您自己的家庭公网 IP 提高安全性）
...
AimiliVPN is a high-performance, zero-dependency VPN proxy gateway built entirely with Python's standard library. It parses official VPNGate servers, benchmarks latency, and provides an Apple-inspired light Web UI for routing traffic through a built-in dual-protocol (HTTP/SOCKS5) proxy server.
...
Open your browser and navigate to the printed URL (for example, `http://your_vps_ip:8787/u71e9IXp4TPx`) to use the Apple-inspired light Web UI.

#### Step 2: Select Node and Mode
1. Wait for the program to complete its first automatic node speed benchmarks.
2. Click **Update Nodes** to trigger node fetching. The backend concurrently tests official VPNGate nodes and ranks them by latency.
